...
export const CustomSelect = ({ label,nameAsset,addAsset}: { label?: any,nameAsset:any,addAsset:any}) => {
  ...
  useEffect(()=>{
    ...
    const textAssets=list[list.length-1].text + " " + list[list.length-1].subText;
    nameAsset(textAssets)
    addAsset(list[list.length-1].text)
  ...
  const handleItem = (item?: any) => {
    ...
    nameAsset(item.text + " " + item.subText)
  };
